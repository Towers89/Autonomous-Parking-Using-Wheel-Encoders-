...
void BLE_UART_Init()
{
    ...
    // Set Bit 6 in the DIR register for P1
    P1->SEL0 &= ~0x40;
    P1->SEL1 &= ~0x40;
    P1->DIR  |= 0x40;
    ...
    // Use one stop bit by clearing the UCSPB bit (Bit 11) in the CTLW0 register
...
{
    //Check the recieve interrupt flag (UCRXIFG, bit 0)
    // in the IFG register and wait if the flag is not set
    // if the UCRXIFG is set, then the recieve buffer (UCAxRXBUF) has
    //received a complete character
    while((EUSCI_A3->IFG & 0x01) == 0);

    // return the data from the recieve buffer (UCAxRXBUF)
...
int BLE_UART_InPacket(char *buffer_pointer, uint16_t buffer_size)
{
    if (buffer_size < 6)
    {
        return 0;
    ...
    buffer_pointer[0] = character;
    buffer_pointer[1] = BLE_UART_InChar();
    buffer_pointer[2] = BLE_UART_InChar();
    buffer_pointer[3] = BLE_UART_InChar();
    buffer_pointer[4] = BLE_UART_InChar();
    buffer_pointer[5] = '\0';
